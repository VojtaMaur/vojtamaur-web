...
### 6.3.1 Image metadata and privacy

Image files may contain embedded metadata such as EXIF, XMP, IPTC, camera information, timestamps, software history, thumbnails, or GPS coordinates.

Because this repository is public and `public/` assets are copied into the build output unchanged, image metadata must be treated as public data once committed. Stripping metadata only during the build is not sufficient, because the original source file may still remain available in the repository, mirrors, releases, archive snapshots, or external copies.

Default rule:

* image files committed to the public repository should have metadata removed before commit
...
For normal image publishing, the recommended workflow is:

1. keep the original file in a private archive, if needed
2. export or copy a public version of the image
3. remove metadata from the public version
4. commit only the cleaned public version into `public/images/`

Example command using ExifTool:

    exiftool -all= -overwrite_original public/images/example.jpg

For files where metadata is intentionally preserved, this should be a conscious exception rather than the default publishing behavior.
...
- image files in the public repository should be checked for embedded metadata before commit, especially GPS and other privacy-sensitive fields
