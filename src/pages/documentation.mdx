...
- `npm run build:web:translate` – creates the web build, fills missing English translation cache entries through DeepL, generates `dist/ALL_POSTS.txt`, generates the reconstructable source package, and writes unsigned build integrity metadata for `dist/`
- `npm run build:web:translate:signed` – runs the translating web build and then signs and verifies its checksum manifest
