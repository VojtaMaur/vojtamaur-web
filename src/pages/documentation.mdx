...
The repository root contains `export-all.bat`. It switches the Windows console and Python standard streams to UTF-8, changes to the repository directory, runs the four manual exports sequentially, and stops immediately if any command fails. The ultra export runs before the metaweb export, so the combined PDF receives the freshly generated ultra PDF rather than a stale copy:
...
### 9.6.6 Gemini capsule and Gopher map generation
