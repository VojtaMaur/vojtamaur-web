...
- `npm run build:web` – creates the standard production web build, applies the English postprocess using existing cache only, and generates `dist/ALL_POSTS.txt`
- `npm run build:web:translate` – creates the web build, fills missing English translation cache entries through DeepL, and generates `dist/ALL_POSTS.txt`
- `npm run build:web:strict` – creates the web build, fails if an English translation cache entry is missing, and generates `dist/ALL_POSTS.txt`
- `npm run preview` – local preview of the build
- `npm run build:usb` – creates a portable file-based build, applies the English postprocess, rewrites paths for offline use, and generates `dist/ALL_POSTS.txt`
- `npm run build:usb:translate` – creates a portable file-based build, fills missing English translation cache entries, and generates `dist/ALL_POSTS.txt`
- `npm run build:arweave` – creates the strict standard web build, generates `dist/ALL_POSTS.txt`, and then creates `dist-arweave/` as a Permaweb-compatible deployment copy with rewritten paths
...
This command first runs the strict standard web build and then runs:
...
The `build:arweave` script is defined as a strict standard web build followed by Arweave-specific postprocessing. The effective order is:

```text
npm run build:web:strict && node scripts/make-arweave-build.mjs
```

This implies four main steps:
...
3. `scripts/generate-all-posts.mjs` creates `dist/ALL_POSTS.txt`
...
5. Test the build locally under a fake manifest-like subdirectory.
6. Upload only `dist-arweave/` to ArDrive.
7. Create the manifest at the level where `index.html` is directly present.
8. Test the manifest gateway URL.
9. Check several deep routes, English routes, `ALL_POSTS.txt`, and `ARCHIVE.txt`.
...
3. optionally test `dist-arweave/` locally under a fake manifest-like subdirectory
4. upload only `dist-arweave/` to a public ArDrive drive
5. create an Arweave manifest at the level where `index.html` is directly present
6. copy the manifest Data TX ID
7. test the site through an Arweave or ArDrive gateway URL
