...
The repository root contains `export-all.bat`. It switches the Windows console and Python standard streams to UTF-8, changes to the repository directory, runs the six manual exports sequentially, and stops immediately if any command fails. The ultra export runs before the metaweb PDF export, so the combined PDF receives the freshly generated ultra PDF rather than a stale copy. The workflow then creates the practical compact Metaweb EPUB and separate compact Czech and English site EPUBs:
...
The manual SSTV PNG export is run separately with `npm run export:sstv`; it is not included in `export-all.bat` (section 9.6.8).
...
It creates numbered RGB PNG pages at the native resolution of the selected standard SSTV mode. Each PNG represents one complete frame. The exporter is started explicitly with `npm run export:sstv`; it is not called by any normal web, USB, Arweave, or Gemini build, or by `export-all.bat`.
...
- further development of the SSTV (Slow-Scan Television) edition: manual PNG export, batch PD120 WAV encoding, volume joining, and visual recovery through MMSSTV have been implemented and tested (section 9.6.8); calibration images and a systematic comparison of the source and received pages remain to be added
