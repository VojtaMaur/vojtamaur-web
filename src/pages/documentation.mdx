...
The third command is the recommended practical complete-site export. It creates separate Czech and English EPUB files with compact images. Because the plain `compact` preset favors the smallest practical files, animated GIFs are represented by static first-frame posters; add `--gif-mode preserve` only when retaining their animation is more important than output size.
...
The recommended command uses static first-frame posters for animated GIFs to keep the otherwise complete bilingual Metaweb publication compact. Add `--gif-mode preserve` when animation is required.
...
- creation of a long-form, unedited audiovisual walkthrough of the entire website, with author commentary and additional context for individual works and sections
