...
- `npm run generate:all-posts` – generates `dist/ALL_POSTS.txt`, a plain-text export of all article content from the finished build output, and embeds the same text into `dist/404.html`
...
- `npm run export:epub` – manually creates separate Czech and English reflowable EPUB books from the finished article pages in `dist/`, including a generated title/index section; this command is not part of the normal build pipeline
- `npm run export:epub:metaweb` – manually creates one bilingual Metaweb EPUB with both article languages, its image appendix, archive map, technical documentation, and build identity files; this command is not part of any build pipeline
...
- `npm run export:pdf:metaweb` – manually creates one archival PDF containing the bilingual metaweb article and its directly related preservation files; this command is not part of any build pipeline
- `npm run export:sstv` – manually renders indexed articles from the finished `dist/` build to numbered PNG frames at native SSTV mode dimensions; defaults to Czech PD120 output under `exports/sstv/pd120/`, does not run a build or generate audio, and is not part of any build pipeline (section 9.6.8)
...
<meta name="archive-index" content="https://vojtamaur.cz/ARCHIVE.txt" />
```

The tags are rendered by `src/layouts/BaseLayout.astro` only when `pathForCs === "/"`, which identifies both homepage variants. Both languages use the same absolute public URLs: the metaweb article explaining the archive, the full-text export, and the archive index. These metadata tags do not embed the contents of those resources.
...
- writes `vojtamaur-web-export-pdf.manifest.json` with article metadata, output file sizes, and SHA-256 hashes
...
- the Czech page `/metawebovy-clanek/`
- the postprocessed English page `/en/metawebovy-clanek/`
- a deduplicated appendix of every image linked from the article, with the physical archive ID and item name plus Czech and English descriptions derived from the corresponding article versions, the source filename, and public link
- a bilingual explanatory page followed by `exports/vojtamaur-web-export-ultra.pdf`, the ultra-compact Czech export of every article with its images
- `ARCHIVE.txt`
- the complete rendered technical documentation at `/documentation/`
- every file linked from the article section `INTEGRITA A IDENTITA BUILDU`, including the checksum manifest, detached signature when present, build checksum, JSON metadata, signing status, and public OpenPGP key
...
Install their Python dependencies once. Pillow 11.3 or newer is required so
that any AVIF image discovered in a finished build can be decoded and converted
to an EPUB core image format:
...
- places every article in its own XHTML spine document with the correct `lang` and `xml:lang`
- generates EPUB 3 package metadata, manifest, spine, navigation document, and stable reflowable CSS
...
- keeps downloads, PDFs, demos, and other non-image payloads as public `https://vojtamaur.cz/` links
- replaces iframes and other interactive embeds with labelled public links because a non-scripted reflowable EPUB cannot preserve their runtime
...
- `Missing, remote or unsupported image source` means an image could not be resolved to an available local build asset. An empty `source` value can come from a `MediaRow` item written as `{ type: "image", src: "" }`. Use `{ type: "empty" }` for an intentionally blank cell, as described in section 7.2. The exporter renders unavailable images as `[Image unavailable]`; correctly rendered empty cells are skipped. After correcting the MDX, rebuild the web and then export again. Otherwise the old `<img>` elements remain in `dist/` and the same warnings return.
- `Cannot render image: ...` includes the reason why an existing image could not be decoded or rasterized. Check the named source and any required image-rendering dependencies.
- `Missing font glyphs rendered as explicit Unicode code points` lists characters that the available fonts could not render. The PNG uses visible text such as `[U+FFFD]`; the manifest retains the original text. This warning groups affected characters rather than reporting every occurrence separately. `U+FFFD` is the Unicode replacement character (`�`), which can already occur in experimental article content such as Anti-jazyk. Its presence does not by itself establish that the SSTV exporter damaged the text.
...
- `vm:sourceMetadata` is a JSON literal (`@type: "@json"`); its original index keys are not interpreted as Schema.org terms. The index's `TITLE` may remain Czech even for an English article, while `headline` comes from the rendered language version.
- `vm:position` records article order starting at 1, since ordinary JSON-LD arrays do not imply RDF ordering. Each language version counts separately in `vm:articleCount`.
- `vm:builtHtmlSha256` hashes the whole original HTML file; `vm:sourceIndexSha256` hashes the exact input index bytes. `vm:declaredBuildSha256`, when present, only copies the declared hash from `BUILD_SHA256.txt`; the exporter does not verify the manifest or OpenPGP signature.
- `vm:generatedAt` is the JSON export time, not an article modification date. `datePublished` retains the index date, including legacy first-of-month migration values described in section 5.5.1.
- `vm:languageStatus` records the existing English fallback convention based on the `noindex` marker. It is not an independent translation-quality assessment, and English records may contain Czech ALT text or untranslated passages.
- `vm:sourcePath` records the source path from the finished index. The exporter does not mix the current MDX contents into an older build.
...
By default, the script selects the JSON-LD snapshot with the newest `vm:generatedAt` from `exports/ALL_POSTS.json`, dated `exports/ALL_POSTS-YYYY-MM-DD.json` snapshots, or their `.jsonld` equivalents. `--input` selects an explicit snapshot. Only records with `inLanguage == "cs"` are translated, in `vm:position` order. English is also translated from Czech for this edition; the existing English article records are not reused. The 2026-09-23 source contained 82 Czech articles and 82 English counterparts; later exports can have different counts.
...
`--output exports/rosetta-NAME` selects another shared directory under `exports/`; pass that option consistently when preparing, submitting and collecting that edition. A different source snapshot or model requires a different output directory. For example:

```bash
node scripts/export-site-rosetta.mjs prepare --input exports/ALL_POSTS-2026-09-23.json --output exports/rosetta-2026-09-23
node scripts/export-site-rosetta.mjs run --input exports/ALL_POSTS-2026-09-23.json --output exports/rosetta-2026-09-23
```

#### Failed items and recovery
...
```bash
node scripts/export-site-rosetta.mjs run --retry-failed
```
...
- evaluation of emerging long-term archival media (e.g. [NanoFiche](https://nanofiche.com/)) as costs and availability improve
...
- evaluation of future opportunities for inclusion in lunar cultural archives (e.g. the [Lunar Codex](https://www.lunarcodex.com/)), if suitable public submission pathways become available
- evaluation of future opportunities for inclusion in space-based cultural archives (e.g. the [Arch Mission Foundation](https://www.archmission.org/)), if suitable public submission pathways become available
...
- evaluation of [FOREVER](https://www.forever.com/) as an additional long-term digital preservation service, complementing the existing use of [Permanent.org](https://www.permanent.org/) and providing further redundancy through an independent archival provider
...
The manual exporter `scripts/export-wayback-snapshots.py` submits the site's live sitemap URLs to Internet Archive Wayback Machine Save Page Now (SPN2) and exports confirmed snapshot links. It requires Python 3.10 or newer and uses only the standard library; no additional Python packages are needed.

Every run downloads `https://vojtamaur.cz/sitemap-index.xml` and follows all referenced sitemap files, including nested indices and compressed sitemaps. It processes every unique page URL without language, section, or path filtering, including `/ns/.../`. Exact duplicate URLs are submitted once per run. The sitemap check on 2026-09-24 found 199 unique URLs, including 25 `/ns/` URLs; these counts are observations, not fixed limits.

The input is the published site, so a local build is not required. The script does not discover mirror URLs or other pages outside the sitemap. Submit those separately. This workflow is started manually and is not included in `export-all.bat` or any web, USB, Arweave, or Gemini build command.

##### Internet Archive credentials

Sign in with the usual Internet Archive account at [Internet Archive S3 API keys](https://archive.org/account/s3.php) to obtain the **access key** and **secret key**. These are the account's programmatic credentials. Supply them through these environment variables:

- `IA_ACCESS_KEY_ID` — the access key
- `IA_SECRET_ACCESS_KEY` — the secret key

The script authenticates SPN2 requests with these keys. It does not ask for an email address or account password, read `.env` files, or automatically load the Internet Archive CLI configuration. Keep actual keys out of source files, documentation, committed batch files, and the repository.

The familiar `ia configure` login is related: it asks for an email address and password, then saves the S3 keys in a local configuration file for the `ia` CLI. An existing CLI login alone therefore does not configure this exporter; both environment variables still need to be set. See the [Internet Archive configuration documentation](https://archive.org/developers/internetarchive/configuration.html).

##### Run from Windows Command Prompt (CMD)

Replace the two placeholder values with the actual keys and run all three commands in the same CMD window:
...
These `set` commands apply to the current CMD session and the programs started from it. They do not persist after the window is closed; set both values again in a new window. The script can be launched from any working directory because it resolves the export directory relative to its own location.

For PowerShell, use its environment-variable syntax instead:
...
To check the live sitemap and list the selected URLs without credentials, captures, or output files, run:

```bat
python "G:\vojtamaur-web\scripts\export-wayback-snapshots.py" --dry-run
```

##### Snapshot TXT output and `ARCHIVE.txt`

Each capture run creates a new UTF-8 file in `G:\vojtamaur-web\exports\`, with a timestamped filename such as:

```text
exports/vojtamaur-web-wayback-20260924-143025-123456.txt
```

The TXT contains only confirmed Wayback snapshot URLs, one per line, with no headings, status messages, or blank separator lines. For example:

```text
https://web.archive.org/web/20260420175521/https://vojtamaur.cz/barva-tvar-ton/
```

Each link uses the capture timestamp and final `original_url` returned by SPN2 after success. If the source page redirects, the final URL may differ from the sitemap entry. The script does not construct a success link merely because a capture job was accepted. If SPN2 returns `favicon.ico` for a request that was not itself for `favicon.ico`, the script retries the original URL once after a 15-second wait, within the existing retry limit. If the result is again a favicon, it reports an error, omits that result from the TXT, and counts the URL as failed. Other redirects remain accepted, including `/pf2026/` to `/demos/pf2026/`; this is a targeted favicon check, not a general verification of redirected destinations.

Successful lines are appended, flushed, and synced to disk immediately. Progress, job IDs, errors, and the final summary appear only in the console. The `[current/total]` counter identifies the URL being attempted; the final summary reports successful captures, failed captures, and sitemap errors separately.

After reviewing the results, manually copy the desired lines into `public/ARCHIVE.txt`. The exporter never edits `ARCHIVE.txt` itself. The `exports/` directory is already ignored by Git and lies outside `dist/`.

##### Waiting, errors, and interruption

URLs are processed sequentially, with a five-second pause between them. After SPN2 accepts a job, the script polls its status every five seconds until success or an error. A job that remains pending has no overall time limit; the console periodically reports that it is still waiting.

HTTP `429` and `5xx` responses are retried up to six times after the initial request. The exponential wait starts at 15 seconds and is capped at 300 seconds, unless the server's `Retry-After` header requests a longer wait. Individual network requests have a 60-second timeout. Selected temporary SPN2 job errors also trigger retries. If the reply to a capture submission is lost in a network failure, its outcome is reported as unknown rather than blindly submitting the same job again.

A failed URL is reported and processing continues with the next one; failed or unconfirmed captures are omitted from the TXT. A failed child sitemap is also reported while other referenced sitemaps continue to be processed. Check the final summary to distinguish a complete batch from partial results. A disk-write failure stops the run.

Press `Ctrl+C` to interrupt the run. Completed TXT lines remain available, but the current unconfirmed job may still finish at Internet Archive without appearing in the file. Starting the script again creates a new file and processes the live sitemap from the beginning: there is no automatic resume or skipping based on previous TXT files. Exit codes are `0` for a complete run, `1` for partial results or failure, and `130` for interruption.
