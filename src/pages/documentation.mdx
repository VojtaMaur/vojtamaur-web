...
If the development server and the final build disagree, the build output is authoritative. Translation, `Lang` filtering, strict checks, and USB path rewriting should be verified through build commands and `npm run preview`, not only through dev mode.
